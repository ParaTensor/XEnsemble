...
function connectionIconColor(health, testing) {
  if (testing || health?.status === 'testing') return 'text-[#5F6368]';
...
function FetchModelsButton({ fetching, disabled, onClick, iconClassName = 'w-3.5 h-3.5' }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`${consoleIconButtonClass} text-[#5F6368]`}
      title={label}
      aria-label={label}
    >
      {fetching ? (
...
function ProviderFormFields({
  ...
  return (
    <div className={`${consoleCardClass} bg-[#F4F5F6]/70 p-4 space-y-4`}>
      {!isEdit && (
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-[#5F6368]">One model ID per line.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <div className="space-y-5">
        <div className="rounded-md border border-[#E8EAED] bg-[#F4F5F6] px-3 py-3 space-y-3">
                    ...
                    return (
                      <tr key={p.name} className={consoleTableBodyRowClass}>
                        <td className={`${consoleTableBodyCellClass} min-w-0`}>
                          ...
                          {p.models?.length > 0 ? (
                            <p className="text-xs text-[#5F6368] truncate" title={detailTitle || undefined}>
            ...
            <form id="gateway-process-form" onSubmit={handleSaveProcess} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-9 min-h-9 py-1.5 font-mono"
                />
                <p className="text-xs text-[#5F6368]">Agents reach the LLM proxy at this base URL (optional; defaults to localhost).</p>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-9 min-h-9 py-1.5 font-mono"
                />
                <p className="text-xs text-[#5F6368]">Leave empty to use the embedded local UniGateway process.</p>
              </div>
