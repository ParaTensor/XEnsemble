...
fn merge_openai_tool_call_arguments(existing: &str, incoming: &str) -> String {
    ...
    for overlap in (1..=max_overlap).rev() {
        if existing.is_char_boundary(existing.len() - overlap)
            && incoming.is_char_boundary(overlap)
            && existing[existing.len() - overlap..] == incoming[..overlap]
        {
...
mod tests {
    ...
    #[test]
    fn merge_multibyte_utf8_no_panic() {
        let existing = "{\"query\":\"长沙\"}";
        let incoming = "沙\"}";
        let result = merge_openai_tool_call_arguments(existing, incoming);
        assert_eq!(result, existing);

        let existing2 = "{\"path\":\"/关";
        let incoming2 = "关}";
        let _ = merge_openai_tool_call_arguments(existing2, incoming2);
    }
}
