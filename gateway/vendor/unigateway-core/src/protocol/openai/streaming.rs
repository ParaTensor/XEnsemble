        ...
        if let Err(join_error) = handle.await {
            let msg = if join_error.is_panic() {
                ...
                if let Some(s) = panic_payload.downcast_ref::<String>() {
                    ...
                    "unknown panic in openai stream processing".to_string()
                }
            } else {
                "openai stream processing task cancelled".to_string()
            };
        ...
        if let Err(join_error) = handle.await {
            let msg = if join_error.is_panic() {
                ...
                if let Some(s) = panic_payload.downcast_ref::<String>() {
                    ...
                    "unknown panic in responses stream processing".to_string()
                }
            } else {
                "responses stream processing task cancelled".to_string()
            };
            let _ = panic_tx.send(Err(GatewayError::StreamAborted {
                message: msg,
                endpoint_id: panic_endpoint_id,
